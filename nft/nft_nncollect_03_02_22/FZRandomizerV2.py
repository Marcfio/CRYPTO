...
def genRandomCode(props, unique):
    ...
    while x < len(props):
        ...
        selObj = random.randint(0, objCount-1)
...
class FZR_Panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row = layout.row()
        row.label(text="Active Object: " + obj.name, icon='BLENDER')
        ...
        row.label(text=str(bpy.context.scene.possibleGen))
...
def register():
    ...
    bpy.types.Scene.spawnfloat_x = bpy.props.FloatProperty(name = "", default=0.1)
    ...
    bpy.types.Scene.possibleGen = bpy.props.IntProperty(name = "Possibilities", default = 0)
